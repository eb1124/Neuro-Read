      ...
      <div className="flex flex-wrap justify-center gap-4 mb-16">
        {subModes.map((m) => (
          <button
            ...
            className={`flex items-center gap-3 px-6 py-4 rounded-2xl transition-all duration-300 border ${subMode === m.id
            ...
            <motion.span
              ...
              onClick={async () => {
                try {
                  ...
                  if (result.success) {
                    const { meaning, example, pronunciation } = result.data;

                    alert(
                      `📚 Meaning: ${meaning}\n\n📝 Example: ${example}\n\n🔊 Pronunciation: ${pronunciation}`
                    );
                  } else {
                    alert("Failed to fetch meaning");
                  }
                } catch (err) {
                  console.error(err);
        ...
        <button
          onClick={async () => {
            try {
              ...
              console.log("SIMPLIFY RESULT:", data);
            ...
            <div className="grid grid-cols-6 gap-2">
              {LETTERS.map((l, idx) => (
                <button
                  ...
                  className={`w-full aspect-square rounded-xl flex items-center justify-center font-bold text-lg transition-all border ${letterIdx === idx
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 py-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-8">
        ...
        <div className="p-12 bg-white rounded-[3.5rem] border border-moss/5 shadow-sm min-h-[200px] flex flex-wrap gap-6 items-center justify-center">
          {keywords.length > 0 ? (
            keywords.map((word, i) => {
              const icon = iconMap[word] || 'ph:question-bold';
            ...
            <p className="text-moss/20 italic">Start typing to see visuals...</p>
          )}
        </div>
      </div>
    </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {['bat', 'cat', 'rat', 'mat'].map(w => (
              <button
                ...
                className={`p-6 rounded-2xl border font-bold text-xl transition-all
    ${selected === w
                    ? w === correctAnswer
                      ? "bg-green-500 text-white border-green-500"
                      : "bg-red-500 text-white border-red-500"
                    : "bg-white border-blue-100 text-blue-900 hover:border-blue-400 hover:bg-blue-600 hover:text-white"
                  }
  `}
              >
                {w}
    ...
    return (
      <div className="animate-in fade-in duration-500">
        <button
          ...
          className="mb-8 text-clay font-medium hover:underline"
        >
          ← Back to Practice
        </button>

        <div className="max-w-xl mx-auto text-center bg-orange-50/50 rounded-[3rem] border border-orange-100 p-10">
          <h3 className="text-2xl font-medium text-moss mb-6">Word Builder</h3>

          {/* Built word */}
          <div className="text-4xl tracking-widest mb-6 min-h-[50px]">
            {builtWord || "_ _ _"}
          </div>
          ...
          <div className="flex justify-center gap-4 mb-6 flex-wrap">
            {letters.map((l, i) => (
              <button
                key={i}
                onClick={() => setBuiltWord(prev => prev + l)}
                className="p-4 bg-white border rounded-xl text-xl font-bold hover:bg-orange-500 hover:text-white"
              >
                {l}
              </button>
            ))}
          ...
          {builtWord.length > 0 && (
            <p className="font-bold text-lg">
              {builtWord === currentWord
                ? "✅ Correct!"
                : builtWord.length === currentWord.length
                  ? "❌ Wrong!"
                  : "Keep going..."}
            </p>
          )}
...
function StoryModeSection({ userId }) {
  ...
  // 🔴 STORY VIEWS
  if (selectedStory === "ant") {
    return (
      <div className="p-10 max-w-3xl mx-auto">
        <button onClick={() => setSelectedStory(null)} className="mb-6 underline">
          ← Back
        </button>

        <h2 className="text-3xl font-bold mb-4">The Brave Little Ant</h2>
        <p className="text-lg leading-relaxed text-moss">
          Once upon a time, a tiny ant found a huge cookie in the grass.
          It was too heavy to carry alone. The ant called its friends.
          Together, they worked as a team and carried the cookie home.
          That night, they all shared a big feast happily.
        </p>
      </div>
    );
  }

  if (selectedStory === "fox") {
    return (
      <div className="p-10 max-w-3xl mx-auto">
        <button onClick={() => setSelectedStory(null)} className="mb-6 underline">
          ← Back
        ...
        <h2 className="text-3xl font-bold mb-4">The Clever Fox</h2>
        <p className="text-lg leading-relaxed text-moss">
          A clever fox was very hungry but couldn’t reach the food high up.
          Instead of giving up, the fox used its intelligence to find another way.
          It solved the problem step by step and finally got what it wanted.
          The fox proved that thinking smart is better than giving up.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 py-12 text-center">
      ...
      <p className="text-text-muted max-w-xl mx-auto mb-10">
        Stories that adjust to your reading level, providing just the right amount of challenge.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
        ...
        {/* Ant Story */}
        <div
          onClick={() => setSelectedStory("ant")}
          className="p-10 bg-purple-50/50 border border-purple-100 rounded-[3rem] relative overflow-hidden group hover:bg-white transition-all cursor-pointer"
        >
          <span className="bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest mb-4 inline-block">
            Easy
          </span>
          <h4 className="text-2xl font-medium text-purple-900 mb-2">
            The Brave Little Ant
          </h4>
          <p className="text-sm text-purple-900/60">
            A short story with simple words and teamwork.
          </p>
        </div>

        {/* Fox Story */}
        <div
          onClick={() => setSelectedStory("fox")}
          className="p-10 bg-purple-50/50 border border-purple-100 rounded-[3rem] relative overflow-hidden group hover:bg-white transition-all cursor-pointer"
        >
          <span className="bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest mb-4 inline-block">
            Easy
          ...
          <p className="text-sm text-purple-900/60">
            A story about solving problems using smart thinking.
          </p>
        </div>

        {/* Locked */}
        <div className="p-10 bg-moss/5 border border-moss/10 rounded-[3rem] opacity-50 grayscale">
          <h4 className="text-2xl font-medium text-moss mb-2">
            Mystery at the Farm
          </h4>
          <p className="text-sm text-text-muted">
            Complete Level 1 to unlock more stories!
          </p>
        </div>
