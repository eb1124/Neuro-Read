...
def build_chunks(text: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    # Paragraph-based chunking, grouping sentences into readable blocks so that
    # multi-sentence paragraphs still yield more than one chunk to page through.
    import re
    ...
    for p in paragraphs:
        ...
        current_len = 0
        for s in sents:
            if current_len + len(s) + 1 > TARGET_LEN and current:
