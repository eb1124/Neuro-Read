...
export default function AssistiveMode({ active, onOpenSimplifier, onRunSimplifier, onSetInputText }) {
  ...
  const handleOcrExtraction = async (imageSource) => {
    ...
    try {
      ...
      setDocResult({
        ...
        metrics: simplified.metrics || { cognitive_load: 'N/A' },
        keywords: simplified.keywords || extractedKw
      });
  ...
  return (
    <div
      ...
      className={`col-start-1 row-start-1 transition-all duration-700 ease-spring ${active ? 'opacity-100 translate-y-0 z-10' : 'opacity-0 translate-y-8 pointer-events-none z-0'
        }`}
    >
