...
export default function InteractiveReader({
  ...
  const navLength = mode === 'chunk' ? (chunks?.length || 0) : (guidedSentences?.length || 0);
    ...
    <div>
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setActiveIdx((v) => Math.min(navLength - 1, v + 1))}
            disabled={activeIdx >= navLength - 1}
            className="text-[10px] px-3 py-1 rounded-full bg-white border border-moss/15 text-charcoal/60 disabled:opacity-50"
          >
            Next
          </button>
